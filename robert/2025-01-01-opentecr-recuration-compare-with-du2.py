...
if RELOAD or not "df_original" in globals():
...
if RELOAD or not "df2_original" in globals():
...
for i, row in du.iterrows():
    ...
    if len(matches) == 1:
        du.loc[i, "matching comment"] = "full match, based on reference_code+Temperature+pH+Kprime"
        print("full match, based on reference_code+Temperature+pH+Kprime")
        ...
        print(">1 match for this reference_code+Temperature+pH+Kprime")
        for x in ["part", "page", "col l/r", "table from top", "entry nr"]:
        ...
        if len(matches) == 0 and du.loc[i, "matches"] > 0:
            continue
        elif len(matches) <= du.loc[i, "matches"]:
            du.loc[i, "matches"] = len(matches)
            for x in ["part", "page", "col l/r", "table from top", "entry nr"]:
                if len(matches[x].unique()) == 1:
                    du.loc[i, x] = matches.iloc[0][x]
            if len(matches) == 1:
                du.loc[i, "matching comment"] = "full match for this reaction+reference_code+Temperature+pH+Kprime"
                print("full match for this reaction+reference_code+Temperature+pH+Kprime")
                others -= 1
                perfect_match += 1
                continue
            else:
                du.loc[i, "matching comment"] = ">1 match for this reaction+reference_code+Temperature+pH+Kprime"
                print(">1 match for this reaction+reference_code+Temperature+pH+Kprime")
                continue
        else:
            continue
    elif len(matches) == 0:
        ...
        if len(matches) == 1:
            ...
            print("1 match for this reference_code+Temperature+pH")
            for x in ["part", "page", "col l/r", "table from top", "entry nr"]:
                du.loc[i, x] = matches.iloc[0][x]
            ...
            if len(matches) == 0 and du.loc[i, "matches"] > 0:
                ...
                if len(matches) == 1:
                    ...
                    print("full match for this reaction+reference_code+Temperature+pH")
                    others -= 1
                    perfect_match += 1
                    continue
                else:
                    du.loc[i, "matching comment"] = ">1 match for this reaction+reference_code+Temperature+pH"
                    print(">1 match for this reaction+reference_code+Temperature+pH")
                    continue
            else:
                continue
        elif len(matches) == 0:
            ...
            if len(matches) == 1:
                ...
                print("1 match for this reference_code+Temperature")
                for x in ["part", "page", "col l/r", "table from top", "entry nr"]:
                    du.loc[i, x] = matches.iloc[0][x]
                tecrdb.loc[matches.index, "matched"] += 1
                perfect_match += 1
                ...
                if len(matches) == 0 and du.loc[i, "matches"] > 0:
                    ...
                    if len(matches) == 1:
                        ...
                        print("full match for this reaction+reference_code+Temperature")
                        others -= 1
                        perfect_match += 1
                        continue
                    else:
                        du.loc[i, "matching comment"] = ">1 match for this reaction+reference_code+Temperature"
                        print(">1 match for this reaction+reference_code+Temperature")
                        continue
                else:
                    continue
            elif len(matches) == 0:
                ...
                if len(matches) == 1:
                    ...
                    print("1 match for this reference_code")
                    for x in ["part", "page", "col l/r", "table from top", "entry nr"]:
                        du.loc[i, x] = matches.iloc[0][x]
                    tecrdb.loc[matches.index, "matched"] += 1
                    perfect_match += 1
                    ...
                    if len(matches) == 0 and du.loc[i, "matches"] > 0:
                        ...
                        if len(matches) == 1:
                            ...
                            print("full match for this reaction+reference_code")
                            others -= 1
                            perfect_match += 1
                            continue
                        else:
                            du.loc[i, "matching comment"] = ">1 match for this reaction+reference_code"
                            print(">1 match for this reaction+reference_code")
                            continue
                    else:
                        continue
                elif len(matches) == 0:
                    du.loc[i, "matching comment"] = "0 matches for this reference_code"
                    print("0 matches for this reference_code")
                    zero_matches += 1
                    continue
...
for i, row in du.iterrows():
    ...
    if len(matches) == 1:
        ...
        print("full match, based on reference_code+Temperature+pH+enthalpy")
        for x in ["part", "page", "col l/r", "table from top", "entry nr"]:
            du.loc[i, x] = matches.iloc[0][x]
        tecrdb.loc[matches.index, "matched"] += tecrdb.loc[matches.index, "matched"] + 1
        perfect_match += 1
        ...
        print(">1 match for this reference_code+Temperature+pH+enthalpy")
        for x in ["part", "page", "col l/r", "table from top", "entry nr"]:
            if len(matches[x].unique()) == 1:
                du.loc[i, x] = matches.iloc[0][x]
        others+=1
        ...
        if len(matches) == 0 and du.loc[i, "matches"] > 0:
            ...
            if len(matches) == 1:
                ...
                print("full match for this reaction+reference_code+Temperature+pH+enthalpy")
                others -= 1
                perfect_match += 1
                continue
            else:
                du.loc[i, "matching comment"] = ">1 match for this reaction+reference_code+Temperature+pH+enthalpy"
                print(">1 match for this reaction+reference_code+Temperature+pH+enthalpy")
                continue
        else:
            continue
    elif len(matches)==0:
